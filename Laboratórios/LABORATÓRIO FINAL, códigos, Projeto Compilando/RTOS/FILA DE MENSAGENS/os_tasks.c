	...
	while (1) {
		...
		if (statusMutex == kStatus_OSA_Success) {
			...
			case 2:
				...
				case kStatus_OSA_Success: // turn off All LEDs
					ledrgb_clearRedLed();
					ledrgb_clearGreenLed();
					ledrgb_clearBlueLed();
					OSA_TimeDelay(2000);
...
void Task2_task(os_task_param_t task_init_data) {
	...
	while (1) {
		...
		if (statusMutex == kStatus_OSA_Success) {
			...
			case 1:
				statusMsg = OSA_MsgQGet(handler, &messageToGet, 2000); // espera 2 segundos

				switch (statusMsg) {
				case kStatus_OSA_Success: // pisca Verde 5x
				...
				case kStatus_OSA_Timeout: //pisca Branco 10x
					for (int i = 0; i < 10; i++) {
						ledrgb_setBlueLed();
						ledrgb_setRedLed();
						ledrgb_setGreenLed();
			...
			case 2:
				statusMsg = OSA_MsgQGet(handler, &messageToGet, 2000);

				switch (statusMsg) {
				case kStatus_OSA_Success: // pisca Amarelo 5x
					for (int i = 0; i < messageToGet.piscaLed; i++) {
						ledrgb_clearRedLed();
						ledrgb_clearGreenLed();
						ledrgb_clearBlueLed();
				...
				case kStatus_OSA_Timeout: // pisca bracno 10x
					for (int i = 0; i < 10; i++) {
						ledrgb_setBlueLed();
						ledrgb_setRedLed();
						ledrgb_setGreenLed();
